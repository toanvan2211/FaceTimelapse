...
class FaceImage:
    ...
    def save(self, outputpath, imagenumber):
        ...
        # Extract date from filename (assuming filename format "IMG_YYYYMMDD_HHMMSS")
        filename = os.path.splitext(self.filename)[0]  # Remove extension
        parts = filename.split('_')
        date_str = parts[1]  # Get the date part "YYYYMMDD"
        ...
        try:
            ...
            self._log(f"Error formatting date: {e}")
        ...
        textSizes = [cv.getTextSize(text, font, fontScale, lineType)[0] for text in texts]

        # Position settings
        marginX = 50  # X margin
        startY = self._finalImg.shape[0] - 50  # Start Y position (bottom margin)
        lineHeight = int(sum(ts[1] for ts in textSizes) * 1.2)  # Line height including gap

        # Draw texts on the image
        for i, text in enumerate(texts):
            textX = marginX
            textY = startY - i * lineHeight
            cv.putText(self._finalImg, text, (textX, textY), font, fontScale, fontColor, lineType)
...
def runFaceImage(imagepath, outpath, imagenumber):
